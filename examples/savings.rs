...
fn main() {
    ...
    let (r1, e1) = iter(&mut mtd, 100, 1500, true); // optimum: 12
    let (r2, e2) = iter(&mut mtd, 1000, 750, true); // optimum: 16
    let (r3, e3) = iter(&mut mtd, 100, 1500, false); // optimum: 8
    let (r4, e4) = iter(&mut mtd, 1000, 750, false); // optimum: 16
